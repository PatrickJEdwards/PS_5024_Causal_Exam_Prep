...
- For the sample of polling stations, there is still only **one** definable (unconditional) average treatment effect (ATE) estimand, where $\tau_{ij}$ is each of the 6 unit treatment effects defined above:
  
$$
\tau_{ATE} = \frac{1}{8} \sum^8_{i=1} \left( \frac{1}{6} \sum^6_{j=1} \tau_{ij} \right)
$$
